...
static void  ACC_SafeDistance_Atomic_ContractFailures_contractFailed(uint8_t contractID, char *loc);
...
void  ACC_SafeDistance_Atomic____ssd_safe_distance_computation_execute(ACC_SafeDistance_Atomic____ssd_safe_distance_computation_data_t *___id, double v_acc_demo, double v_lead, double *d_safe) 
{
  ...
  
  {
    ...
    if ( !__assertCondition ) 
    {
      ...
      ___BlockMessages____block_messagelist_Contract_Failed("ACC_SafeDistance_Atomic_ssd_safe_distance_computation_samplePre", "BlockInterfaces:ssd_safe_distance_computationInterface_newBlock:1?r:f599b715-12fa-44c1-a288-a72fd0bffe50(_30_Architecture)#6504848816738695791");
      printf("v_lead=%f\n",v_lead);;
      printf("v_acc_demo=%f\n",v_acc_demo);;
    }
  }
  ...
  (*d_safe) = (v_acc_demo * ___id->T_SAFE_hatchback) + (fabs(v_lead - v_acc_demo) * 0.2) + (D_BRAKE_sedan);
  ...
  {
    bool __assertCondition;
    __assertCondition = (*d_safe) >= 3.0;
    if ( !__assertCondition ) 
    {
      ___BlockMessages____block_messagelist_Contract_Failed("ACC_SafeDistance_Atomic_ssd_safe_distance_computation_valid_d_safe", "BlockInterfaces:ssd_safe_distance_computationInterface_newBlock?r:f599b715-12fa-44c1-a288-a72fd0bffe50(_30_Architecture)#3980660603150638789");
      printf("d_safe=%f\n",(*d_safe));;
    }
  }
  {
    ...
    if ( !__assertCondition ) 
    {
      ___BlockMessages____block_messagelist_Contract_Failed("ACC_SafeDistance_Atomic_ssd_safe_distance_computation_temporalCondition", "BlockInterfaces:ssd_safe_distance_computationInterface_newBlock:2?r:f599b715-12fa-44c1-a288-a72fd0bffe50(_30_Architecture)#3980660603150649888");
      printf("v_acc_demo=%f\n",v_acc_demo);;
      printf("v_lead=%f\n",v_lead);;
      printf("d_safe=%f\n",(*d_safe));;
    }
  ...
  {
    bool __assertCondition;
    __assertCondition = (*d_safe) >= 5.0;
    if ( !__assertCondition ) 
    {
      ___BlockMessages____block_messagelist_Contract_Failed("ACC_SafeDistance_Atomic_ssd_safe_distance_computation_valid_d_safe", "BlockInterfaces:ssd_safe_distance_computationInterface_newBlock:3?r:f599b715-12fa-44c1-a288-a72fd0bffe50(_30_Architecture)#6427938858666181797");
      printf("d_safe=%f\n",(*d_safe));;
    }
  }
}
...
void  ACC_SafeDistance_Atomic____ssd_distance_error_execute(ACC_SafeDistance_Atomic____ssd_distance_error_data_t *___id, double d_lead_hatchabck, double d_safe_new, double *d_error) 
{
  ...
  {
    bool __assertCondition;
    __assertCondition = (d_lead_hatchabck > 0.0) && (d_lead_hatchabck < 300.0);
    if ( !__assertCondition ) 
    {
  ...
  (*d_error) = (d_safe_new - d_lead_hatchabck) / 1.0;
  if ( (*d_error) > 3000.0 ) 
  {
    (*d_error) = 3000.0;
...
void  ACC_SafeDistance_Atomic____ssd_mode_compute_execute(void *___nothing, double d_lead_hatchabck, double d_safe_new, double *ssd_mode) 
{
  ...
  if ( d_lead_hatchabck > d_safe_new ) 
  {
    /* 
