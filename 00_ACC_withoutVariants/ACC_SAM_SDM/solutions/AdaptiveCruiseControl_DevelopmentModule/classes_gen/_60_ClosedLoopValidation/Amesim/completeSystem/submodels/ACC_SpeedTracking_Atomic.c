 ...
 */
void  ACC_SpeedTracking_Atomic____st_mode_computation_execute(void *___nothing, double d_brake, double d_lead_hatchabck, double d_safe_new, double v_acc, double v_set, double *st_mode) 
{
  
  
  
  
  (*st_mode) = ACC_SpeedTracking_Atomic_st_mode_table(v_acc, v_set, d_lead_hatchabck, d_safe_new, d_brake);
  
  
  
  {
    bool __assertCondition;
    __assertCondition = !((d_lead_hatchabck > d_safe_new)) || ((*st_mode) == 1.0);
