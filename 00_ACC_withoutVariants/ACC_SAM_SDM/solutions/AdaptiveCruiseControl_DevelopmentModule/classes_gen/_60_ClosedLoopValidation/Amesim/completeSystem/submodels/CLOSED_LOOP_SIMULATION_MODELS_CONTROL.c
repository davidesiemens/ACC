...
void closed_loop_simulation_models_control_(int *n ,double *ACC_switch, double *d_lead_hatchabck, double *v_acc, double *v_lead, double *v_ref, double *brake, double *d_brake, double *d_safe_new, double *throttle, double rp[1], double c[1], int  ic[5], int *flag, double *t)
   ...
   if (*flag == 0)
   {
	   if(*t >= c[0])
	   {
		   if (c[0] <= (getfinaltime_() - rp[0]))
		   {
			   ...
			   c[0] = c[0] + rp[0];
			   Closed_Loop_Simulation_Models____control_flattened_execute( blockStruct ,*ACC_switch,*d_lead_hatchabck,*v_acc,*v_lead,*v_ref,brake,d_brake,d_safe_new,throttle);
		   }
	   }
   }
   distim_(&c [0]);
